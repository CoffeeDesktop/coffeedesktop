...
class Templates
  ...
  desktop_object: ->
    "<div id='desktop_object_{{uuid}}' class='desktop_object' style='left:{{x}}px;top:{{y}}px'>
      <img style='width:48px;'' src='vendors/icons/{{img}}'>
  ...
  dock_object: (uuid,icon,fullname,name,options) ->
    "<li ><img  id='app#{uuid}' src='vendors/icons/#{icon}' alt='#{fullname}' title='#{fullname}' desktop_object_options='#{options}' desktop_object_run='#{name}' desktop_object_fullname='#{fullname}' desktop_object_icon='#{icon}' class='dockItem i_wanna_be_a_desktop_object dock_app app_#{name}' />"
  dock_drop_up: ->
...
class Backend
  ...
  fetchApps: ->
    apps = $.getJSON("vendors/apps.json", (apps) =>
      apps.every (app) => 
       @fetchApp(app)
    )
